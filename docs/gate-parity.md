...
| `publish.yml`               | adopt   | A release path that is not the gate's build path is a package nobody checked.                                                                                                                                                                                                                                                                                 | present as `publish.yaml`, which builds through this repository's own `build-run.yml`, the workflow the gate calls, and refuses to publish a package whose entries differ from the artifact the gate uploaded for the tagged commit, from [#35](https://github.com/Flowfin/jellyfin-plugin-discover/issues/35) and [#119](https://github.com/Flowfin/jellyfin-plugin-discover/issues/119); [#124](https://github.com/Flowfin/jellyfin-plugin-discover/issues/124) checks the published package from the operator's side |
...
| `manifest-carries-the-release.yml` | keep            | The manifest a server installs from is written and served by the Flowfin catalogue, so a publish that leaves it listing the previous version, or this one with a checksum the package does not have, leaves no trace here; this reads the published bytes daily against the newest release, per [#120](https://github.com/Flowfin/jellyfin-plugin-discover/issues/120) and [#124](https://github.com/Flowfin/jellyfin-plugin-discover/issues/124). It runs on no pull request and publishes no name on one, so it has no row in the third table. |
...
Nine jobs here declare that they follow another:
    ...
    .github/workflows/publish.yaml:411:    needs: gate
    .github/workflows/publish.yaml:418:    needs: build
    .github/workflows/publish.yaml:518:    needs: [build, compare]
    .github/workflows/publish.yaml:544:    needs: [build, compare, attest]
    .github/workflows/publish.yaml:648:    needs: [gate, build, compare, attest, release]
...
| `Documented commands still print what is pasted`           | yes                   | It reads the tree deterministically, and off the mainline it reads a block naming `origin/master` against both commits rather than deferring it.  |
