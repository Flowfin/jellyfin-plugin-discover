...
| Installing a certificate into the machine trust store, or disabling certificate validation globally | Outlives the run and weakens the machine, or removes the property under test while leaving the test green.                  | One injected handler in front of every outbound call, supplied by the test, so no test needs a real endpoint. It is built and it is reached: the plugin registers one named client, `PluginServiceRegistrator` resolves that client's primary handler out of the container, and `OneInjectedHandlerInFrontOfEveryCallTests` registers `AHandlerThatRefusesWhatNoTestSetUp` there and drives the adapter through it, with a second substitute on the default client so a call made through the wrong one is refused rather than made. Beside it: `TmdbSourceAdapterTests.AConnectionThatCouldNotBeVerifiedFailsAndIsNotAskedAgain` hands the adapter a connection that did not verify and asserts the fetch ends there rather than asking again, four tests around `DefaultDeadline` cover the timeout, and `SourceRestTests` and `ARefusedSourceIsLeftAloneTests` cover retry and backoff against instants a test declares. Those four groups reach the adapter's transport function rather than the handler, one layer inside the client, because what they judge is what the adapter makes of a reply. | [#45](https://github.com/Flowfin/jellyfin-plugin-discover/issues/45)         |

The third replacement is built now, and what the row says about it is narrower
than "done". The handler exists, a test replaces it, and every outbound call the
plugin makes goes through the one client it is registered on; what no test in
this repository holds is that a real endpoint presents a certificate this runtime
accepts, which is under what none of this covers below. The row is written that
way rather than as a tick, because a table where every row reads as done is the
shape that makes a plan look like a suite.

What was missing shrank twice and the row moved with it each time, on 2026-08-31
and again when the handler landed; a row that had gone on naming three absences
where none was left would have been read as three, which is the same drift in the
