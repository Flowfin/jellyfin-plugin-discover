...
| Installing a certificate into the machine trust store, or disabling certificate validation globally | Outlives the run and weakens the machine, or removes the property under test while leaving the test green.                  | One injected handler in front of every outbound call, supplied by the test, so no test needs a real endpoint. Part of it is built: `TmdbSourceAdapterTests.AConnectionThatCouldNotBeVerifiedFailsAndIsNotAskedAgain` hands the adapter a connection that did not verify and asserts the fetch ends there rather than asking again. Retry and backoff are covered too, since `SourceRest` landed: `SourceRestTests` asserts the rule on the type that decides it and `ARefusedSourceIsLeftAloneTests` asserts what a run does with it, both against instants a test declares. Neither reaches the injected handler, because a rest is computed from what an adapter reported rather than from a reply a transport gave. A timeout is covered by nothing, and [#78](https://github.com/Flowfin/jellyfin-plugin-discover/issues/78) is what brings it. | [#45](https://github.com/Flowfin/jellyfin-plugin-discover/issues/45)         |

The third replacement is named and not wholly built, and the row says which part
is missing. That is stated rather than elided, because a table where every row
reads as done is the shape that makes a plan look like a suite, and a row that
reads as done because most of it is would be the same shape one step quieter.
What is left of it shrank on 2026-08-31 and the row moved with it; a row that
had gone on naming three absences where one was left would have been read as
three, which is the same drift in the direction that flatters nobody.
