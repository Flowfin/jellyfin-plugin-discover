...
public class AnUninstallTakesWhatThisPluginWroteTests
{
    ...
    public void AnUninstallLeavesTheConfigurationDocumentWhereTheServerPutIt()
    {
        ...
        try
        {
            ...
            var document = Path.Combine(paths.PluginConfigurationsPath, "Discover.xml");
    ...
    private static string ARootOfItsOwn() =>
        Path.Combine(
            Path.GetTempPath(),
            "jellyfin-plugin-discover-tests",
            "uninstall-" + Guid.NewGuid().ToString("n", System.Globalization.CultureInfo.InvariantCulture));
